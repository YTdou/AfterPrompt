...
演示预览和统一导出的 HTML 不恢复导入页面原有脚本。生成流程再次净化文档，并把页面源码放入不允许脚本的内层 `sandbox="allow-same-origin"` iframe。只有编辑器生成的外层播放代码可以运行，用于缩放、翻页、页码和全屏。

统一 HTML 使用带版本标记的 inert `<template>` 保存 Base64 JSON payload。导入器在 sanitizer 之前只提取并校验该数据，不执行导出文件中的 Runtime；随后仍对恢复出的规范源码应用正常安全清洗。旧版 Last Mile Studio Slides 兼容路径只解析固定的 `sourceBase64` 数据声明，不执行旧脚本。
